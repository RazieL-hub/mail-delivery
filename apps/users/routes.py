...
@router.put('/{user_id}')
async def update_user(user: UserSchema, db: Session = Depends(get_session)):
    query = text(get_user_query).bindparams(user_id=user.user_id)
    cursor = await db.execute(query)
    check_user = cursor.fetchone()
    if not check_user:
        raise HTTPException(status_code=404, detail="user not found")
    query = text(update_user_query).bindparams(**user.dict())
    # query = text(f"""update users set {params} where user_id={check_user.user_id}""")
    await db.execute(query)
    await db.commit()

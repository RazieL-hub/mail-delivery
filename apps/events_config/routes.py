...
from apps.events_config.querys import get_event_config_query, add_settings_for_user_query, delete_settings_query, \
    update_settings_query, get_list_send_settings_query
from apps.events_config.schemas import EventSetting, EventSettingUpdateSchema
...
@router.get('/{user_id}', response_model=List[EventSetting])
...
@router.get('/{user_id}/{type_event}', response_model=EventSetting)
async def get_detail_user_settings(user_id: int, type_event: str, db: Session = Depends(get_session)):
    query = text(get_event_config_query).bindparams(user_id=user_id, type_event=type_event)
    cursor = await db.execute(query)
...
@router.post('/add')
async def add_settings_for_user(user_settings: EventSetting, db: Session = Depends(get_session)):
    ...
    query = text(get_event_config_query).bindparams(user_id=user_settings.user_id, type_event=user_settings.type_event)
    cursor = await db.execute(query)
    user = cursor.fetchone()
    if user:
        raise HTTPException(status_code=404,
...
@router.delete('/{user_id}/{type_event}', response_model=EventSetting)
...
@router.put('/{user_id}/{type_event}', response_model=EventSettingUpdateSchema)
async def update_settings_for_user(user_id: int, type_event: str, user_settings: EventSettingUpdateSchema,
